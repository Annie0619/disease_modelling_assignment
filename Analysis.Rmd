...
```{r}
# ---- validation: tidy data + publication-ready plots (saves to outputs/) ----
# Creates two figures:
#  1) Observed, Expected (literature rho), Expected (scaled rho_hat) on one panel
#  2) Observed vs Expected (scaled rho_hat) only — cleaner view when lit line is far off
# Also writes a CSV snapshot of the plotted time series.

# Ensure outputs dir exists
out_dir <- "outputs"
if (!dir.exists(out_dir)) dir.create(out_dir, recursive = TRUE)

# Build a tidy table for plotting & export
# Join obs, expected (literature rho), and expected (scaled rho_hat)
val_tbl <- fit_compare_locked %>%
  dplyr::left_join(
    fit_compare_scaled %>% dplyr::select(year, exp_scaled),
    by = "year"
  ) %>%
  dplyr::arrange(year)

# Long format for plotting; keep a key for mapping labels
val_long <- val_tbl %>%
  tidyr::pivot_longer(
    cols = c(obs, exp_locked, exp_scaled),
    names_to = "series_key",
    values_to = "value"
  ) %>%
  dplyr::mutate(
    series_key = factor(series_key, levels = c("obs","exp_locked","exp_scaled"))
  )

# Legend labels:
#  - Both are estimates of rho; one is literature-based (\hat{rho}_{lit}), the other is scaled (\hat{rho}_{scaled})
lab_obs <- "Observed"
lab_lit <- expression("Expected (" * hat(rho)[lit] * ")")
lab_scl <- expression("Expected (" * hat(rho)[scaled] * ")")

# Common style (bigger text, cleaner theme)
base_size <- 16
legend_text_size <- 14
axis_text_size <- 14
line_size <- 1.05
...
# Helper: y-axis as thousands with a label
scale_y_thousands <- scale_y_continuous(
  name   = "Reported cases (thousands)",
  labels = function(x) scales::comma(x / 1000)
)

# ---------- Figure 1: Observed, Expected(\hat{rho}_{lit}), Expected(\hat{rho}_{scaled})
p_all <- ggplot(val_long, aes(x = year, y = value)) +
  geom_line(aes(linetype = series_key), linewidth = line_size) +
  geom_point(
    data = dplyr::filter(val_long, series_key == "obs"),
    size = point_size
  ) +
  scale_linetype_manual(
    name   = NULL,
    values = c("obs" = "solid", "exp_locked" = "dashed", "exp_scaled" = "dotdash"),
    breaks = c("obs","exp_locked","exp_scaled"),
    labels = list(lab_obs, lab_lit, lab_scl)
  ) +
  scale_y_thousands +
  labs(
    title = "Observed vs expected reported cases",
    subtitle = "Stationary demography; no vaccination in fit window",
    x = "Year"
  ) +
...
ggsave("plots/validation_obs_expected_all.png",
       plot = p_all, width = 8, height = 5, dpi = 400)

# ---------- Figure 2: Observed vs Expected(\hat{rho}_{scaled}) only (cleaner)
val_long_scaled <- dplyr::filter(val_long, series_key %in% c("obs","exp_scaled"))

p_scaled <- ggplot(val_long_scaled, aes(x = year, y = value)) +
  geom_line(aes(linetype = series_key), linewidth = line_size) +
  geom_point(
    data = dplyr::filter(val_long_scaled, series_key == "obs"),
    size = point_size
  ) +
  scale_linetype_manual(
    name   = NULL,
    values = c("obs" = "solid", "exp_scaled" = "dotdash"),
    breaks = c("obs","exp_scaled"),
    labels = list(lab_obs, lab_scl)
  ) +
  scale_y_thousands +
  labs(
    title = "Observed vs expected",
    subtitle = expression(paste("Both use ", hat(rho), "; curve shows ", hat(rho)[scaled], " (mean-level rescale)")),
    x = "Year"
  ) +
  theme_bw(base_size = base_size) +
  theme(
...
ggsave("plots/validation_obs_expected_scaled.png",
       plot = p_scaled, width = 8, height = 5, dpi = 400)


# ---------------------------
# Export the numbers used in the plot(s)
# ---------------------------
readr::write_csv(val_tbl, file.path(out_dir, "validation_series_table.csv"))

# Console message
message("Saved: ",
        file.path(out_dir, "validation_obs_expected_all.png"), " ; ",
        file.path(out_dir, "validation_obs_expected_scaled.png"), " ; ",
        file.path(out_dir, "validation_series_table.csv"))
```
