...
```{r}
# Re-read abridged life table (only needed columns)
lt_path  <- "data/raw/WPP2024_Life_Table_Abridged_Medium_2024-2100.csv.gz"
proc_dir <- "data/processed"
mort_years <- 2025:2035  # or just start_year if you prefer baseline-only
...
lt_za <- lt_small %>%
  ...
  dplyr::select(Time, sex, age_start, span_raw, qx, mx)

# Continuous hazard per band; set span for open-ended band to reach age 100
lt_haz <- lt_za %>%
  dplyr::mutate(
    # span: if open-ended (NA), extend to age 100 inclusive
...
# Keep one row per age (0..99) within each year–sex slice
lt_0_99 <- lt_expanded %>%
  dplyr::group_by(year, sex) %>%
  dplyr::arrange(age, .by_group = TRUE) %>%
  dplyr::distinct(age, .keep_all = TRUE) %>%  # replaces slice_unique()
  dplyr::ungroup()

# Create an age-100 row by copying the last hazard in each (year, sex)
lt_age100 <- lt_0_99 %>%
  dplyr::group_by(year, sex) %>%
