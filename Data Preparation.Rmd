...
```{r}
# ---- Binning definitions (Option B) ----
# Single-year: 0..14
single_years <- 0:14

# Five-year bands: 15-19, 20-24, ..., 70-74, and open-ended 75+
five_lower <- seq(15, 70, by = 5)
five_upper <- five_lower + 4

# Build bin table
age_bins_tbl <- tibble::tibble(
  bin_index = seq_along(c(single_years, five_lower, 75)),
  lower     = c(single_years, five_lower, 75),
  upper     = c(single_years, five_upper, Inf),
  label     = c(
    as.character(single_years),
    paste0(five_lower, "-", five_upper),
    "75+"
  )
) %>%
  mutate(
    # Mark bins that intersect ages 15..44 (for CRS later)
    CRS_relevant = (lower <= 44 & upper >= 15)
  )

# ---- Helper: map single-year ages to bin_index ----
...
# ---- Aggregate the processed single-year populations into bins ----
# Input here is wpp_pop (year, age, sex, pop) from the previous chunk
pop_binned <- wpp_pop %>%
...
# Quick sanity: each (year, sex) should sum the same as the single-year totals
check_totals <- pop_binned %>%
  group_by(year, sex) %>%
  summarise(pop_binned = sum(pop), .groups = "drop") %>%
  left_join(
    wpp_pop %>% group_by(year, sex) %>% summarise(pop_single = sum(pop), .groups = "drop"),
    by = c("year", "sex")
  ) %>%
  mutate(diff = pop_binned - pop_single)

print(check_totals %>% mutate(ok = abs(diff) < 1e-6)) # all match- good.

stopifnot(all(abs(check_totals$diff) < 1e-6))

# ---- Save binned file ----
out_file_binned <- file.path(out_dir, "pop_binned.csv")
readr::write_csv(pop_binned, out_file_binned)
message("Saved: ", out_file_binned)
...
```{r}
# Paths
raw_path_fert <- "data/raw/WPP2024_Fertility_by_Age1.csv.gz"  # adjust extension if needed
out_dir       <- "data/processed"
dir.create(out_dir, showWarnings = FALSE, recursive = TRUE)
